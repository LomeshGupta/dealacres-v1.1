  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setProperties(data); // Assuming the API response is an array of property objects
      } catch (error) {
  ...
  return (
    <div className="px-[5rem] py-[3rem] w-full rounded-xl max-md:px-[4rem] max-sm:px-5">
      <div className="flex flex-row w-full">
        ...
        <PropertyFlatList properties={properties} />

        {compareProperties.length > 0 && <ProjectCompareSidebar />}
      </div>
      <Insights />
      <InterestingReads />
      <FNQ />
