...
const Card = ({ index, imageUrl, heading }) => {
  return (
    <div
      key={index}
      className="min-w-[80px] md:min-w-[88px] md:max-w-[90px]  flex flex-col items-center justify-center gap-2 p-2 rounded-xl bg-white text-black">
  ...
  return (
    <div className="bg-[#c5dff8] p-4 md:p-8 ">
      ...
      <div className="flex justify-center">
        <div className="flex flex-row overflow-x-auto gap-4 md:gap-5  cursor-pointer xl:max-w-[1100px] xl:mx-auto ">
          {cardData.map((card, index) => (
            <Link key={index} href={card.path ? card.path : "/"}>
              <Card
                key={index}
      ...
      <div className="flex justify-center mt-3 md:mt-6">
        <Link
          href="/allservice"
          className="px-2 py-0.5 md:px-4 md:py-2 bg-blue-500 text-white text-[0.73rem] md:text-base rounded-md hover:bg-blue-600 focus:outline-none focus:bg-blue-600">
          View All
        </Link>
      </div>
    </div>
